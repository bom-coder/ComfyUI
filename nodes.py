...
class LoadLatent:
    ...
    def INPUT_TYPES(s):
        ...
        files = [f for f in os.listdir(input_dir) if
                 os.path.isfile(os.path.join(input_dir, f)) and f.endswith(".latent")]
...
class CheckpointLoaderSimple:
    @classmethod
    def INPUT_TYPES(s):
        return {"required": {"ckpt_name": (folder_paths.get_filename_list("checkpoints"),), }}  # 文件下拉选址模型

    RETURN_TYPES = ("MODEL", "CLIP", "VAE")  # 返回元组
    FUNCTION = "load_checkpoint"  # 执行调用

    CATEGORY = "loaders"  # 实际分类

    def load_checkpoint(self, ckpt_name, output_vae=True, output_clip=True):
        ckpt_path = folder_paths.get_full_path("checkpoints", ckpt_name) # 模型完整路径
        # 加载模型 输出vae 输出 clip 指定embedding_directory目录
...
class DiffusersLoader:
    ...
    def load_checkpoint(self, model_path, output_vae=True, output_clip=True):
        ...
        return comfy.diffusers_load.load_diffusers(model_path, output_vae=output_vae, output_clip=output_clip,
                                                   embedding_directory=folder_paths.get_folder_paths("embeddings"))
...
class LatentBlend:
    ...
    def blend(self, samples1, samples2, blend_factor: float, blend_mode: str = "normal"):
        ...
        if samples1.shape != samples2.shape:
            ...
            samples2 = comfy.utils.common_upscale(samples2, samples1.shape[3], samples1.shape[2], 'bicubic',
                                                  crop='center')
